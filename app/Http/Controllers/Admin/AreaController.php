...
class AreaController extends Controller
{
    public function index()
	{
		...
		return view('admin.area.add', compact('categories', 'settings'));
	}
	...
	public function insert(Request $request) 
	{
		if(!empty($request->category)) {
			...
			if($request->hasFile('image'))
			{
				...
				if($check) {
					if($request->image->getSize() <= 1048576) {
						$imageName = 'area_'.time().'.'.$request->image->extension();
			...
			if($request->hasFile('banner_image'))
			{
				...
				if($check) {
					if($request->banner_image->getSize() <= 1048576) {
						$imageName = 'area_banner_image_'.time().'.'.$request->banner_image->extension();
			...
			try {
				...
				return redirect()->back()->with('success_message', 'Area added successfully');
			}catch(\Exception $e) {
	...
	public function list() {
		$posts = DB::table('posts')
					...
					->where('posts.post_type', 'area')
					->where('status', '<>', 'deleted')
					->get();
		$categories = DB::table('category_posts')->where('status', '<>', 'deleted')->get();
		$settings = DB::table('settings')->first();
		return view('admin.area.list', compact('posts', 'categories', 'settings'));
	}
	...
	public function edit($id) {
		...
		$settings = DB::table('settings')->first();
		return view('admin.area.edit', compact('categories', 'post', 'category_arr', 'categories_elements', 'custom_fields', 'settings'));
	}
	...
	public function update(Request $request, $id) {
		...
		try {
			...
			return redirect()->back()->with('success_message', 'Area updated successfully');
		}catch(\Exception $e) {
			return redirect()->back()->with('success_message', $e->getMessage());
		}
	}
