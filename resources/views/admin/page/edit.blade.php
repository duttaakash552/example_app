	...
	<div class="wrapper">
		...
		<div class="page-wrapper">
			<div class="page-content">
					...
					<div class="ms-auto">
						<div class="btn-group">
							...
							<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">
								<a class="dropdown-item" href="<?= route('admin.page.list'); ?>">Go To List</a>
							</div>
						</div>
					</div>
				</div>
                    ...
                    <div class="col-lg-8 mx-auto">
						<div class="card">
							...
							<div class="card-body p-4">
								<form id="jQueryValidationForm" method="post" action="<?= route('admin.page.update', $page->id); ?>" enctype="multipart/form-data">
									@csrf
									<div class="row mb-3">
										<label for="input35" class="col-sm-3 col-form-label">Title</label>
										<div class="col-sm-9">
											<input type="text" class="form-control" id="input35" name="title" value="<?= $page->title ?>" placeholder="Enter Page Title" required>
										</div>
									</div>
									<div class="row mb-3">
									...
									<div class="row mb-3">
										...
										<div class="col-sm-9">
											<select class="form-select" id="input36" name="template" required>
												...
												<option value="<?= $template->id ?>" <?= (($template->id == $page->template) ? 'selected' : '') ?>><?= $template->name ?></option>
												<?php } ?>
											</select>
										</div>
									</div>
									<div class="row mb-3">
										<label for="input50" class="col-sm-3 col-form-label">Parent Page</label>
										<div class="col-sm-9">
											<select class="form-select" id="input50" name="parent_id">
									...
									<div class="row mb-3">
										<label for="input51" class="col-sm-3 col-form-label">Add in header</label>
										<div class="col-sm-9">
											<input type="checkbox" id="input51" name="ckeck_menu" <?= (($page->ckeck_menu == '1') ? 'checked' : '' ) ?> value="1"/>
										</div>
									</div>
									<div class="row mb-3">
										<label for="input81" class="col-sm-3 col-form-label">Disable Link</label>
										<div class="col-sm-9">
											<input type="checkbox" id="input81" name="disable_link" <?= (($page->disable_link == '1') ? 'checked' : '' ) ?> value="1"/>
										</div>
									</div>
									<div class="row mb-3">
									...
									<input type="hidden" name="add_remove_page_image" class="add_remove_page_image" value="0" />
									<div class="row mb-3">
										<label for="input37" class="col-sm-3 col-form-label">Image</label>
										<div class="col-sm-9">
											<input type="file" class="form-control" id="input37" name="image" >
										</div>
										<img src="<?= !empty($page->image) ? URL::to('/').'/public/uploads/page/'.$page->image : '' ?>" class="page_image_preview" />
										<input type="button" class="btn btn-danger px-4 remove_page_image" value="Remove" />
									</div>
									<div class="row mb-3">
										<label for="input38" class="col-sm-3 col-form-label">Status</label>
										<div class="col-sm-9">
											<select class="form-select" id="input38" name="status">
												<?php for($i = 0; $i < count($status); $i++) { ?>
												<option value=<?= $status[$i] ?> <?= (($page->status == $status[$i]) ? 'selected' : '') ?>><?= $status[$i] ?></option>
												<?php } ?>
											</select>
										</div>
									</div>
					...
					<div class="col-lg-8 mx-auto">
						<div class="card">
							...
							<div class="card-body p-4 custom-fields">
								<input type="hidden" name="post_type" class="meta_post_type" value="page" />
								<input type="hidden" name="post_id" class="meta_post_id" value="{{ $page->id }}" />
				...
				<?php }else { ?>
					<p>This page is already deleted</p>
				<?php } ?>
